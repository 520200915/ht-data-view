...
let { version, version_lib, library } = require("./package.json");
...
const cdn = {
  ...
  js: [
    ...
    `${CDN_DOMAIN}/echarts/4.7.0/echarts.min.js`
  ]
};
...
module.exports = {
  ...
  chainWebpack: config => {
    ...
    config.resolve.alias
      ...
      .set("@s", path.resolve(__dirname, "./src/styles"))
      .set("@v", path.resolve(__dirname, "./src/pages"))
  ...
  configureWebpack: config => {
    config.externals = {
      ...
      echarts: "echarts"
    }; // 防止将某些 import 的包(package)打包到 bundle 中，而是在运行时(runtime)再去从外部获取这些扩展依赖(用于cdn引入)
    if (isProduction) {
      // 为生产环境修改配置...
